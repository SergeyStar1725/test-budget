...
fun NormalOpenAPIRoute.budget() {
    route("/budget") {
        route("/add/{author?}").post<BudgetAuthorParam, BudgetRecord, BudgetRecord>(info("Добавить запись")) { param, body ->
            respond(BudgetService.addRecord(param, body))
        }
...
data class BudgetAuthorParam(
    @PathParam("ID автора") val id_author: Int
)
